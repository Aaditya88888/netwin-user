...
const ProfileForm = () => {
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    ...
    if (!validateGameId(data.gameId, profile.gameMode || "PUBG")) {
      form.setError("gameId", {
        type: "manual",
        message: `Please enter a valid ${profile.gameMode || "PUBG"} ID`,
      });
      return;
    }
          ...
          <div className="mt-2 flex flex-wrap gap-2 justify-center sm:justify-start">
            ...
            <span
              className={`px-2 py-1 text-xs rounded-full text-white ${
                String(profile.kycStatus) === "approved"
                  ? "bg-green-600"
                  : String(profile.kycStatus) === "pending"
                  ? "bg-yellow-600"
                  : String(profile.kycStatus) === "rejected"
                  ? "bg-red-600"
                  : "bg-gray-600"
              }`}
            >
              KYC: {String(profile.kycStatus).replace("_", " ").toUpperCase()}
            </span>
          </div>
        </div>
      </div>
