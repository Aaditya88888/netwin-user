  ...
  useEffect(() => {
    const fetchLeaderboardData = async () => {
      try {
        ...
        q = query(q, where("updatedAt", ">=", startDate));

        // Apply sorting
        if (sortBy === 'kills') {
          q = query(q, orderBy("kills", "desc"));
        } else if (sortBy === 'wins') {
          q = query(q, orderBy("wins", "desc"));
        } else {
          q = query(q, orderBy("earnings", "desc"));
        }
    ...
    fetchLeaderboardData();
      // We've already handled search in the main filter
              ...
              return (
                <tr key={entry.userId} className="hover:bg-dark-lighter">
                  ...
                  <td className="py-3 px-4 font-rajdhani font-semibold text-green-400">
                    {formatCurrency(entry.earnings, userProfile?.currency || entry.currency)}
                  </td>
                </tr>
              );
            })}
